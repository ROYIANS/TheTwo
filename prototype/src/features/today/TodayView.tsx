...
function OpportunityToday({ state, dispatch }: { state: DemoState; dispatch: Dispatch<AppAction> }) {
  const current = state.lifeStage === "discover" ? "先确认职位对象，再开始研究" : state.lifeStage === "research" && state.research.status !== "done" ? "让 AI 展开这份机会的证据" : "确认实际工作方式";
  return <div className="today-object-layout"><section className="today-focus"><span className="focus-mark"><BriefcaseBusiness size={15} /></span><p className="eyebrow">当前机会 · {state.opportunity?.company}</p><h2>{current}</h2><p>{state.lifeStage === "discover" ? "机会对象已经由你确认。下一步是启动一次研究，把职位、职业约束和证据放在同一条可追溯的判断里。" : state.lifeStage === "research" && state.research.status !== "done" ? "AI 会把岗位要求、你的硬约束和公开证据拆开处理，过程中你可以随时介入。" : "官网和职位描述对工作方式的说法存在差异，这会直接影响你的硬约束。"}</p><PrimaryButton onClick={() => state.lifeStage === "discover" ? dispatch({ type: "start-research" }) : state.research.status !== "done" ? dispatch({ type: "set-view", view: "opportunity" }) : dispatch({ type: "create-communication" })}>{state.lifeStage === "discover" ? "开始这次研究" : state.research.status !== "done" ? "查看研究进度" : state.communication ? "打开沟通草稿" : "准备一次沟通"}</PrimaryButton></section><aside className="today-trace"><div className="trace-heading"><span><Sparkles size={16} />AI 当前任务</span><StatusPill tone={state.research.status === "done" ? "positive" : state.research.status === "running" ? "warning" : "neutral"}>{state.research.status === "done" ? "已完成" : state.research.status === "running" ? "处理中" : "待开始"}</StatusPill></div><TraceList steps={researchTrace} current={state.research.step} status={state.research.status} /></aside><div className="today-rail"><div><h3>这份机会留下的对象</h3><span>只显示已经发生的关系</span></div><ObjectTrail current={state.lifeStage} available={availableLifeStages(state)} onStage={(stage) => dispatch({ type: "set-life-stage", stage })} /></div></div>;
}
