...
export function OpportunityView({ workspace, dispatch, onOpenAgent }: { workspace: OpportunityWorkspace; dispatch: Dispatch<AppAction>; onOpenAgent: () => void }) {
  ...
  return <div className="feature-view opportunity-detail-view"><ObjectHeader eyebrow={`当前机会 · ${stageLabel(workspace.lifeStage)}`} title={<>{opportunity.company}<br /><em>{opportunity.role}</em></>} description={`${opportunity.salary} · ${opportunity.location} · ${opportunity.direction}`} status={<StatusPill tone={workspace.decision ? "positive" : research.status === "done" ? "warning" : "neutral"}>{status}</StatusPill>}><div className="object-links"><Button tone="quiet" type="button" onClick={() => dispatch({ type: "set-view", view: "opportunities" })}><CheckCircleIcon size={14} />全部机会 <ArrowRightIcon size={13} /></Button><Button tone="quiet" type="button" onClick={() => dispatch({ type: "set-view", view: "profile" })}><CheckCircleIcon size={14} />参与判断的职业事实 <ArrowRightIcon size={13} /></Button><Button tone="quiet" type="button" onClick={() => dispatch({ type: "set-view", view: "today" })}><SparkleIcon size={14} />回到今天 <ArrowRightIcon size={13} /></Button></div></ObjectHeader><ObjectTrail current={workspace.lifeStage} available={availableLifeStages(workspace)} onStage={(stage) => dispatch({ type: "set-life-stage", stage })} />{isResearching && <AgentContextEntry label="追问这份研究" detail="让证据、冲突和未知留在同一个判断现场。" context={`${opportunity.company} · 机会研究`} onOpen={onOpenAgent} />}{isResearching ? <ResearchStage workspace={workspace} selected={selected} evidence={evidence} dispatch={dispatch} /> : <LifecycleStage workspace={workspace} dispatch={dispatch} />}</div>;
}
