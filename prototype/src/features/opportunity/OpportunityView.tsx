...
function ResearchStage({ workspace, selected, evidence, dispatch }: { workspace: OpportunityWorkspace; selected?: EvidenceItem; evidence: EvidenceItem[]; dispatch: Dispatch<AppAction> }) {
  ...
  return <div className={`opportunity-workspace ${complete ? "research-complete" : "research-in-progress"}`}>
    <section className="research-main">
      {complete ? <>
        <div className="conclusion-block">
          <div><p className="eyebrow"><span className="focus-dot" />当前建议</p><h2>存在匹配线索，但现实条件仍需确认</h2><p>{workspace.opportunity.role} 已绑定“{workspace.opportunity.direction}”方向。建议先核验薪资、工作方式和团队边界，再决定是否投入完整申请。{workspace.contextNotes.length ? ` 当前还有 ${workspace.contextNotes.length} 条用户补充参与后续更新。` : ""}</p></div>
    ...
    <aside className="opportunity-side">
      <section className="next-object"><p className="eyebrow"><span className="focus-dot" />下一动作</p><h3>{complete ? "准备完整申请决策包" : "先让研究走完"}</h3><p>{complete ? "定制简历、沟通草稿、风险、未知和审核清单会进入同一个工作面；生成后仍由你编辑和决定。" : "研究完成前不会生成公司结论，也不会把演示证据混进当前机会。"}</p>{complete && <PrimaryButton onClick={() => dispatch({ type: "start-application-package" })}>{workspace.applicationPackage?.status === "done" ? "打开申请决策包" : "生成申请决策包"}</PrimaryButton>}</section>
      <section className="decision-object"><p className="eyebrow"><QuestionIcon size={14} />用户决定</p><h3>{workspace.decision ? `已选择${decisionLabel(workspace.decision)}` : "审核材料后再决定"}</h3><p>{workspace.decisionRecord ? workspace.decisionRecord.reason : "申请、暂缓或放弃以及理由会在申请决策包中保存，系统建议不会替你选择。"}</p>{complete && <button type="button" className="decision-package-link" onClick={() => dispatch({ type: "start-application-package" })}>{workspace.decision ? "回看材料与决定" : "进入申请决策包"}<ArrowRightIcon size={14} /></button>}</section>
...
function OpportunityNote({ workspace, dispatch }: { workspace: OpportunityWorkspace; dispatch: Dispatch<AppAction> }) {
  ...
  return <section className="opportunity-note"><div><p className="eyebrow">补充一个只有你知道的事实</p><h3>让新的现场信息进入下一次判断</h3><p>它会作为用户输入保留，不会直接改写已确认职业事实或伪装成公开证据。</p></div><textarea value={note} placeholder="例如：朋友告诉我这个团队最近刚换负责人……" onChange={(event) => setNote(event.target.value)} /><button type="button" disabled={note.trim().length < 6} onClick={() => { dispatch({ type: "add-opportunity-note", text: note }); setNote(""); }}>加入当前机会</button>{workspace.contextNotes.length > 0 && <small>已保留 {workspace.contextNotes.length} 条用户补充</small>}</section>;
}
