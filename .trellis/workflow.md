...
3. **Write through** — persist every durable event before the next material action, then reconcile files and Git on resume or task boundaries
4. **Incremental development** — one task at a time
5. **Capture learnings** — after each task, review and write new knowledge back to spec
6. **CodeGraph first** — when `.codegraph/` exists, use it before broad source search; fall back for missing index results, docs, and config
...
**Write-through**: if the turn creates a durable project decision, scope change, accepted assumption, research conclusion, risk, or next action, it is not a pure direct answer; bind it to a task and persist it before further material work.
[/workflow-state:no_task]
...
Write through every user decision, scope change, accepted assumption, research conclusion, risk, and next action before the next material step. On resume or task switch, reconcile the current request, task, PRD, accepted docs, and Git state first.
...
**Write-through**: persist every durable event before the next material action. Before handoff, task switch, commit, or finish, reconcile decisions, open questions, risks, next action, accepted docs, and Git state.
...
Before archive, reconcile task state, accepted docs, Git state, decisions, open questions, risks, and next action; persist any mismatch first.
