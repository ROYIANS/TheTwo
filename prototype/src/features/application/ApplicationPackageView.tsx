...
export function ApplicationPackageView({ workspace, dispatch }: { workspace: OpportunityWorkspace; dispatch: Dispatch<AppAction> }) {
  ...
  if (applicationPackage.status === "running") return <div className="feature-view application-generating"><ObjectHeader eyebrow="正在准备申请决策包" title={<>先把事实和风险<br /><em>放进同一份材料。</em></>} description="系统正在从已确认职业事实、当前方向、职位要求和研究证据生成草稿。完成前不会记录申请决定。" status={<StatusPill tone="warning">生成中</StatusPill>} /><section className="analysis-surface"><TraceList steps={applicationTrace} current={applicationPackage.step} status={applicationPackage.status} /><button type="button" className="button-quiet cancel-generation" onClick={() => dispatch({ type: "cancel-application-package" })}><XIcon size={14} />取消并回到机会</button></section></div>;
  ...
  return <div className="feature-view application-package-view">
    <ObjectHeader eyebrow={`申请决策包 · ${workspace.opportunity.company}`} title={<>准备好再行动，<br /><em>不是生成完就行动。</em></>} description="岗位定制表达、沟通草稿、风险、未知项和你的决定保存在同一个工作面。所有对外动作仍由你亲自完成。" status={<StatusPill tone={applicationRecorded || reviewComplete ? "positive" : "warning"}>{applicationRecorded ? "申请已记录" : `${reviewed}/${applicationPackage.reviewItems.length} 项已审核`}</StatusPill>}>
