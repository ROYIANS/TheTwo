...
function ConversationView({ state, dispatch }: { state: DemoState; dispatch: Dispatch<AppAction> }) {
  return <div className="onboarding-view"><ObjectHeader eyebrow="职业访谈 · 第一个问题" title={<>先从你在意的<br /><em>事情开始。</em></>} description="这里不是问卷。你可以先说一段不完整的话，AI 会把它整理成候选理解，再由你确认哪些内容值得保留。" /><section className="conversation-surface"><div className="conversation-prompt"><span className="agent-icon"><MessageCircle size={16} /></span><div><strong>如果下一份工作只能先满足三件事，你最不愿意牺牲什么？</strong><p>可以说薪资、工作方式、技术方向、团队状态，或者你还说不清楚的感觉。</p></div></div><textarea aria-label="职业访谈回答" defaultValue="我想继续做复杂前端和工程化，但不想用长期加班换成长。地点希望在上海，最好能有混合办公。" /><div className="conversation-actions"><button type="button" className="button-primary" onClick={() => dispatch({ type: "start-profile-analysis" })}>让 AI 整理这段话 <ArrowRight size={15} /></button><QuietButton onClick={() => dispatch({ type: "show-resume" })}>改用简历开始</QuietButton></div></section></div>;
}
