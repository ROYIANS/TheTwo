...
export function OpportunityIntakeView({ state, dispatch }: { state: DemoState; dispatch: Dispatch<AppAction> }) {
  ...
  return <div className="feature-view opportunity-intake-view">
    <ObjectHeader eyebrow="带入机会" title={<>把你看到的<br /><em>放回你的语境。</em></>} description="先录入你实际看到的正文、来源和时间。解析不会覆盖原始材料，草稿也要由你检查后才能进入。" status={<StatusPill tone={parsed ? "positive" : parsing ? "warning" : "neutral"}>{parsed ? "草稿待确认" : parsing ? "解析中" : "等待材料"}</StatusPill>} />
    <div className="intake-layout">
      <section className="intake-form" aria-labelledby="intake-material-title">
        <div className="intake-section-head"><div><span className="intake-kicker"><FileTextIcon size={14} />原始职位材料</span><h2 id="intake-material-title">保留你实际看到的内容</h2></div><Button type="button" className="intake-example" disabled={parsing} onClick={useExample}>使用演示职位</Button></div>
