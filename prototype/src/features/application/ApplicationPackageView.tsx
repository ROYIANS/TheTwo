...
export function ApplicationPackageView({ workspace, dispatch, onOpenAgent }: { workspace: OpportunityWorkspace; dispatch: Dispatch<AppAction>; onOpenAgent: () => void }) {
  ...
  return <div className="feature-view application-package-view">
    <ObjectHeader eyebrow={`申请决策包 · ${workspace.opportunity.company}`} title={<>准备好再行动，<br /><em>不是生成完就行动。</em></>} description="定制表达、沟通草稿、风险和你的决定都在同一份材料里。对外的每一步仍由你亲自完成。" status={<StatusPill tone={applicationRecorded || reviewComplete ? "positive" : "warning"}>{applicationRecorded ? "申请已记录" : `${reviewed}/${applicationPackage.reviewItems.length} 项已审核`}</StatusPill>}>
      <div className="object-links"><Button tone="quiet" type="button" onClick={() => dispatch({ type: "set-view", view: "opportunity" })}>回到机会判断 <ArrowRightIcon size={13} /></Button><Button tone="quiet" type="button" onClick={() => dispatch({ type: "set-view", view: "today" })}>回到今天 <ArrowRightIcon size={13} /></Button></div>
    </ObjectHeader>
    ...
    <div className="application-layout">
      ...
      <section className="application-decision"><p className="eyebrow">你的最终决定</p><h2>建议谨慎投入。<br />你怎么选？</h2><div className="decision-segments">{(["apply", "hold", "decline"] as const).map((item) => <Button tone="plain" disabled={applicationRecorded} type="button" className={choice === item ? "selected" : ""} key={item} onClick={() => setChoice(item)}>{item === "apply" ? "申请" : item === "hold" ? "暂缓" : "放弃"}</Button>)}</div><label><span>记录你的理由</span><textarea disabled={applicationRecorded} value={reason} placeholder="说明这次选择最重要的依据……" onChange={(event) => setReason(event.target.value)} /></label><Button tone="primary" type="button" className="button-primary" disabled={applicationRecorded || reason.trim().length < 8} onClick={saveDecision}>保存我的决定</Button>{workspace.decisionRecord && <div className="saved-decision"><CheckCircleIcon size={15} /><span><strong>已保存：{workspace.decisionRecord.choice === "apply" ? "申请" : workspace.decisionRecord.choice === "hold" ? "暂缓" : "放弃"}</strong><small>{workspace.decisionRecord.reason}</small></span></div>}{applicationRecorded ? <div className="application-recorded"><CheckCircleIcon size={15} weight="fill" />已记录本人投递</div> : workspace.decision === "apply" && choice === "apply" && <Button tone="primary" type="button" className="record-application" disabled={!reviewComplete} onClick={() => dispatch({ type: "record-application" })}>确认我已亲自投递</Button>}</section>
      </aside>
    </div>
  </div>;
}
