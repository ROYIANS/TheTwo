...
function nextTask(workspace: OpportunityWorkspace, dispatch: Dispatch<AppAction>): { title: string; description: string; action: string; onAction: () => void } {
  ...
  if (!workspace.communication) return { title: "问清会改变判断的事", description: "研究已完成，最有价值的下一步是生成一份只存在于当前机会里的沟通草稿。", action: "准备沟通草稿", onAction: () => dispatch({ type: "create-communication" }) };
  if (!workspace.decision) return { title: "你准备把这份机会推进到哪？", description: "沟通草稿已经准备好。现在由你决定是否申请、暂缓或放弃，系统建议不会替你选择。", action: "回看建议并决定", onAction: () => dispatch({ type: "set-view", view: "opportunity" }) };
  if (!workspace.application && workspace.decision === "apply") return { title: "把申请作为一条事件留下", description: "确认你已经亲自投递后，材料、渠道和当时的建议会与机会绑定。", action: "记录我的申请", onAction: () => dispatch({ type: "record-application" }) };
